...
WHERE i.location_code ~ '^ddm' AND i.is_available_at_library = 'true' AND i.item_status_code = '-'
);

CREATE INDEX copies_index ON available_copies(bib_record_id);
ANALYZE available_copies;WITH available_copies AS(
SELECT
l.bib_record_id,
i.itype_code_num,
i.location_code,
...
FROM
sierra_view.hold h
WHERE h.pickup_location_code ~ '^ddm' AND h.is_frozen = 'false' AND (h.placed_gmt::DATE + h.delay_days) < NOW()::DATE
)hold_num
ON
h.id = hold_num.id
JOIN
...
FROM
sierra_view.hold h
WHERE h.pickup_location_code ~ '^ddm'
)hold_num
ON
h.id = hold_num.id
JOIN
...
ON
h.record_id = ir.id AND ir.location_code ~ '^ddm' AND ir.is_available_at_library = 'true' AND ir.item_status_code = '-'
JOIN
sierra_view.bib_record_item_record_link l
ON
