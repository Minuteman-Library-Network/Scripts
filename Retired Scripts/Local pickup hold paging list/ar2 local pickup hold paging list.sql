...
WHERE i.location_code ~ '^ar2' AND i.is_available_at_library = 'true' AND i.item_status_code = '-'
);
...
FROM
sierra_view.hold h
WHERE h.pickup_location_code ~ '^ar2' AND h.is_frozen = 'false' AND (h.placed_gmt::DATE + h.delay_days) < NOW()::DATE
)hold_num
ON
h.id = hold_num.id
JOIN
...
FROM
sierra_view.hold h
WHERE h.pickup_location_code ~ '^ar2'
)hold_num
ON
h.id = hold_num.id
JOIN
...
ON
h.record_id = ir.id AND ir.location_code ~ '^ar2' AND ir.is_available_at_library = 'true' AND ir.item_status_code = '-'
JOIN
sierra_view.bib_record_item_record_link l
ON
