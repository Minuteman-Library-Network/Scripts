...
Uploads resulting file to staff intranet site
"""
...
def excelWriter(query_results):
    # Name of Excel File
    excelfile = "/Scripts/Cambridge New Titles/Temp Files/CAMNewTitles{}.xlsx".format(
        date.today()
    )
    excelfile2 = (
        "/Scripts/Cambridge New Titles/Temp Files/CAMNewTitlesChildrens{}.xlsx".format(
            date.today()
        )
    )
    ...
    eformatlabelurl = workbook.add_format(
        {
            "text_wrap": True,
            "valign": "bottom",
            "bold": True,
    ...
    eformaturl2 = workbook2.add_format(
        {
            "text_wrap": True,
            "valign": "bottom",
            "font_name": "Arial",
    ...
    eformatlabel2 = workbook2.add_format(
        {
            "text_wrap": True,
            "valign": "bottom",
            "bold": True,
    ...
    eformatlabelurl2 = workbook2.add_format(
        {
            "text_wrap": True,
            "valign": "bottom",
            "bold": True,
    ...
    sftp_file(
        "C:\\Scripts\\Cambridge New Titles\\Temp Files\\CAMNewTitles{}.xlsx".format(
            date.today()
        )
    )
    sftp_file(
        "C:\\Scripts\\Cambridge New Titles\\Temp Files\\CAMNewTitlesChildrens{}.xlsx".format(
            date.today()
        )
    )
...
def sftp_file(local_file):
    ...
    for fname in srv.listdir_attr():
        fullpath = "/reports/Library-Specific Reports/Cambridge/New Titles/{}".format(
            fname.filename
        )
        name = str(fname.filename)
        if name.startswith("CAMNewTitles") and (
...
def main():
    ...
    JOIN sierra_view.item_record_property i
      ON bi.item_record_id = i.item_record_id
    JOIN sierra_view.item_record ir
      ON i.item_record_id = ir.id
    JOIN sierra_view.record_metadata m
      ON ir.id = m.id

    WHERE ir.location_code ~ '^cam'
      AND ir.icode1 NOT IN ('118','219','241','195')
      AND m.creation_date_gmt >= (CURRENT_DATE - INTERVAL '1 month')
    ...
    EXCEPT
      ...
      WHERE ir.location_code ~ '^cam'
	    AND ir.icode1 NOT IN ('118','181','183','184','185','195','202','203','204','205','209','219','239','241')
	    AND m.creation_date_gmt < (CURRENT_DATE - INTERVAL '1 month')
      GROUP BY 2,3,4,1,5,6
      ORDER BY 1
    """

    tempFile = run_query(cam_query)
    ...
    tempFileBranches = run_query(branches_query)
    # Name of Excel File
    excelfile = (
        "/Scripts/Cambridge New Titles/Temp Files/CAMBranchesNewTitles{}.xlsx".format(
            date.today()
        )
    )
    excelWriterBranches(tempFileBranches, excelfile)
    sftp_file(
        "C:\\Scripts\\Cambridge New Titles\\Temp Files\\CAMBranchesNewTitles{}.xlsx".format(
            date.today()
        )
    )
...
if __name__ == "__main__":
    try:
        ...
        email_subject = "Cambridge New Titles script error"
        email_message = (
