...
def excel_writer(query_results, excel_file):
    ...
    worksheet.write(0, 0, "PType", eformatlabel)
    worksheet.write(0, 1, "Label", eformatlabel)
    worksheet.write(0, 2, "Total_Count", eformatlabel)
    worksheet.write(0, 3, "Active_Count", eformatlabel)
    worksheet.write(0, 4, "New_Count", eformatlabel)
...
def main():
    ...
    query = """\
           SELECT
             ...
             COUNT(p.id) FILTER(WHERE rm.creation_date_gmt > (localtimestamp - INTERVAL '1 year')) AS New_Patrons
           FROM sierra_view.patron_record p
           JOIN sierra_view.ptype_property_myuser n
             ON p.ptype_code = n.value
    ...
    excel_file = "/Scripts/Annual Reports/Archive/PatronCountByPtype{}.xlsx".format(
        date.today()
    )
    excel_writer(query_results, excel_file)

    # send email with attached file
    email_subject = "Patron Count By PType"
    email_message = """***This is an automated email***


The Patron Count by Ptype report has been attached."""
...
if __name__ == "__main__":
    try:
        ...
        email_subject = "annual reports: patron count by ptype script error"
        email_message = (
