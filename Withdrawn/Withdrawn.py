...
Generates list of items that have been given a status of withdrawn over a month ago for each library
...
def excel_writer(query_results,excel_file):
    ...
    #Inserting a header
    worksheet.set_header('Withdrawn Items')
...
def main(library,libcode):
    ...
    try:
      query = r"""
              SELECT DISTINCT
                SUBSTRING(i.location_code, 1, 5) AS location_code,
                ip.barcode, 
                it.name AS itype,
                i.icode1 AS scat,
              ...
              WHERE i.item_status_code = 'w'
                AND i.last_status_update >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 month')
                AND i.last_status_update < DATE_TRUNC('month', CURRENT_DATE)
      ...
      email_subject = "Withdrawn " + library + " script error"
      email_message = (
