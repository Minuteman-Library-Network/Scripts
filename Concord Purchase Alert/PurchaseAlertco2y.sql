...
JOIN (
  SELECT 
    ...
    COUNT(DISTINCT h.id) FILTER(WHERE h.pickup_location_code ~ '^co2') AS local_holds,
    ...
    COUNT(DISTINCT CASE WHEN ia.location_code ~ '^co2' THEN ia.id END) AS local_avail_item_count,
    --location will take the form ^oln, which in this example looks for all locations starting with the string oln.
  ...
  LEFT JOIN sierra_view.item_record ia
    ...
    AND ((ia.location_code !~ '^co2' AND ia.itype_code_num NOT IN ('5','21','109','133','160','183','239','240','241','244','248','249'))
    OR ia.location_code ~ '^co2')
   LEFT JOIN (
     SELECT
