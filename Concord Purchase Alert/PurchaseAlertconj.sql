...
JOIN (
  SELECT 
    ...
    COUNT(DISTINCT h.id) FILTER(WHERE h.pickup_location_code ~ '^con') AS local_holds,
    ...
    COUNT(DISTINCT CASE WHEN ia.location_code ~ '^con' THEN ia.id END) AS local_avail_item_count,
    --location will take the form ^oln, which in this example looks for all locations starting with the string oln.
  ...
  LEFT JOIN sierra_view.item_record ia
    ...
    AND ((ia.location_code !~ '^con' AND ia.itype_code_num NOT IN ('5','21','109','133','160','183','239','240','241','244','248','249'))
    OR ia.location_code ~ '^con')
   LEFT JOIN (
     SELECT
...
HAVING mv.local_holds >= 1
  AND COUNT(ir.id) FILTER (WHERE (ir.itype_code_num BETWEEN '150' AND '183' OR SUBSTRING(ir.location_code,4,1) = 'j')) > 0
ORDER BY 5, mv.local_holds DESC
