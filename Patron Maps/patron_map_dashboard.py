...
Queries Sierra for various patron activity metrics, grouped by census tract
and emails it to staff as an attachment
"""
...
def send_email(subject, message, attachment):
    ...
    emailto = config_recipient["patron_map_dashboard"]["recipients"].split()
...
SELECT
  ...
  ROUND(AVG(DATE_PART('year',AGE(CURRENT_DATE,p.birth_date_gmt::DATE))))::INT AS avg_age,
...
The patron map tract report has been attached.

Use to Overwrite the geography.patrons table within BigQuery.
For more information Refer to notes page of census tract choropleth maps dashboard seen when viewing as an editor.
"""
...
if __name__ == "__main__":
    try:
        ...
        email_subject = "patron map dashboard script error"
        email_message = (
