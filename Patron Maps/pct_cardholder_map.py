...
Generates html file containing an interactive choropleth maps showing
the number of cardholders in each census block group along with some additional metrics in a tooltip
...
def gen_map(patron_df, lat, lon, mapzoom):
    ...
    fig.update_layout(
        ...
        title="Cardholder Percentage",
        legend_title="",
    )
    # save map to html file
    pio.write_html(
        fig,
        file="/Scripts/Patron Maps/Temp Files/CardholderPct{}.html".format(
            date.today()
        ),
        auto_open=False,
    )
...
def main(library, tracts, lat, lon, mapzoom):
    try:
        ...
        email_subject = "patron maps: pct cardholder map " + library + " script error"
        email_message = (
...
if __name__ == "__main__":
    ...
    main(
        "Bedford", "'359100','359300','359301','359302','359303'", 42.4932, -71.2766, 11
    )
    main(
        "Belmont",
